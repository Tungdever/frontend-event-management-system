...
function App() {
  ...
  const AppContent = () => {
    ...
    // Kiểm tra nếu là trang login
    const isLoginPage = location.pathname === "/login";

    return (
      <div className="app">
        {/* Hiển thị Sidebar và Topbar nếu không phải trang login */}
        {!isLoginPage && <Sidebar selectedEvent={selectedEvent} setSelectedEvent={setSelectedEvent} />}
        <div className={`content-wrapper ${isLoginPage ? "login-wrapper" : ""}`}>
          {!isLoginPage && <Topbar />}
